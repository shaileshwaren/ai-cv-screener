...
2. For candidates with ai_score >= 80:
   ...
   - Uploads JSON to Airtable (ai_detailed_json field)
   - Uploads HTML to Airtable (ai_report_html field)
...
Output:
  ...
  - Uploads JSON to Airtable (ai_detailed_json)
  - Uploads HTML to Airtable (ai_report_html)
"""
...
# =========================
def update_airtable_detailed_json_and_html(
    airtable: AirtableClient,
    record_id: str, 
    detailed_json: dict,
    html_path: Path
) -> bool:
    """Update the ai_detailed_json and ai_report_html fields in Airtable."""
    
    try:
        # Update JSON field
        success = airtable.update_record(record_id, {
        ...
        resp = airtable.upload_text_as_attachment(
            record_id=record_id,
...
def main() -> int:
    ...
    for candidate in high_scorers:
        ...
        try:
            ...
            if cache_key:
                record_id = airtable.find_record_by_field("cache_key", cache_key)
                if record_id:
                    if update_airtable_detailed_json_and_html(airtable, record_id, detailed_json, html_path):
                        uploaded_count += 1
                        print(f"  ✓ Uploaded to Airtable")
                    else:
                        print(f"  ✗ Airtable upload failed")
                else:
                    print(f"  ⚠ Airtable record not found")
